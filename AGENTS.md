...
### Safe type assertion rules

Direct type assertion on `interface{}` / `any` is FORBIDDEN. It panics on type mismatch.
ALWAYS use the two-value form ("comma, ok idiom") or `switch v := i.(type)`.

BAD (panics and fails `make check`):
```
client := ctx.Value("client").(*http.Client)
id := data.(int)
```

SAFE (compiles and passes checks):
```
client, ok := ctx.Value("client").(*http.Client)
if !ok {
    // Must handle the case where type doesn't match
    return fmt.Errorf("client not found in context or has wrong type")
}
```

Alternatively, use type switch:
```
switch v := data.(type) {
case int:
    // work with int
